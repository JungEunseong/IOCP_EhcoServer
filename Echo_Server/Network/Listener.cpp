...
Listener::Listener(HANDLE iocpHandle) : _iocpHandle(iocpHandle)
{
	...
	WSAIoctl(dummySocket, SIO_GET_EXTENSION_FUNCTION_POINTER, &guid, sizeof(guid), (LPDWORD*)&AcceptEx, sizeof(AcceptEx), &bytes, NULL, NULL);
}

void Listener::OnExecute(IoEvent* event)
{
...
void Listener::RegisterAccept(AcceptEvent* ioEvent)
{
	...
	if (false == AcceptEx(_listenSocket, ioEvent->session->GetSocket(), ioEvent->session->GetRecvBuffer(), 0,
